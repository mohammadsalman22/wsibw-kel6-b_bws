...
<div class="w3l-news" id="news">
    <section id="grids5-block" class="py-5">
        <div class="container py-lg-5 py-md-4 py-2">
          <?php
            $query_mysql = mysqli_query($connect,"SELECT * FROM tb_wisata WHERE nama_wisata LIKE '%".$_GET['s']."%'");
            if ($query_mysql->num_rows == 0) {
          ?>
          <h3 class="title-big text-center">Hasil '<?= $_GET['s']?>' tidak ditemukan.</h3>
          <?php
           } else {
          ?>
            <h3 class="title-big text-center">Hasil pencarian dari '<?= $_GET['s']?>'</h3>
            <?php } ?>
