...
class Library
{
    ...
    public function add_data($nama_siswa, $tgllhr, $alamat)
    {
        $data = $this->db->prepare('INSERT INTO tb_siswa (nama_siswa,tgllhr,alamat) VALUES (?, ?, ?)');
        
        $data->bindParam(1, $nama_siswa);
        $data->bindParam(2, $tgllhr);
    ...
    public function update($kd_siswa,$nama_siswa,$tgllhr,$alamat){
        $query = $this->db->prepare('UPDATE tb_siswa set nama_siswa=?,tgllhr=?,alamat=? where kd_siswa=?');
        
        $query->bindParam(1, $nama_siswa);
        $query->bindParam(2, $tgllhr);
